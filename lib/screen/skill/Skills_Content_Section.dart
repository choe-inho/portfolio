...
class SkillsContentSection extends StatelessWidget {
  const SkillsContentSection({super.key});
  ...
  Widget build(BuildContext context) {
    ...
    return Obx(() {
      if (appController.isMobile) {
        return const _MobileSkillsContent();
      } else {
        return const _DesktopSkillsContent();
      }
    });
  }
}
...
/// 데스크톱/태블릿 스킬 콘텐츠
class _DesktopSkillsContent extends StatelessWidget {
  const _DesktopSkillsContent();
...
/// 모바일 스킬 콘텐츠
class _MobileSkillsContent extends StatelessWidget {
  const _MobileSkillsContent();
...
/// 스킬 카테고리 그리드 (데스크톱/태블릿)
class _SkillCategoryGrid extends StatelessWidget {
  const _SkillCategoryGrid();
  ...
  Widget build(BuildContext context) {
    ...
    return Column(
      children: categorizedSkills.entries.map((entry) {
        ...
        return Padding(
          padding: EdgeInsets.only(bottom: constants.spacingXXL),
...
/// 스킬 카테고리 리스트 (모바일)
class _SkillCategoryList extends StatelessWidget {
  const _SkillCategoryList();
